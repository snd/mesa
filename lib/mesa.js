...
mesa = {
  ...
  baseEmbed: function(records, otherTable, options) {
    var condition;
    condition = {};
    condition[options.otherKey] = 'function' === typeof options.thisKey ? records.map(options.thisKey) : _.pluck(records, options.thisKey);
    return otherTable.where(condition).find().then(function(otherRecords) {
      var grouped;
      grouped = _.groupBy(otherRecords, options.otherKey);
      records.forEach(function(record) {
        var associated, thisValue;
        thisValue = 'function' === typeof options.thisKey ? options.thisKey(record) : record[options.thisKey];
        associated = grouped[thisValue];
        return record[options.as] = options.many ? associated : associated[0];
      });
      return records;
    });
  },
  defaultsForEmbed: function(otherTable, immutableOptions) {
    var options, otherTableName, thisTableName;
    thisTableName = this.getTable();
    otherTableName = otherTable.getTable();
    options = immutableOptions != null ? _.clone(immutableOptions) : {};
    if (options.thisIsForeign == null) {
      options.thisIsForeign = false;
    }
    if (options.thisKey == null) {
      if (options.thisIsForeign) {
        ...
        options.thisKey = this.getPrimaryKey();
      }
    }
    if (options.otherKey == null) {
      if (options.thisIsForeign) {
        ...
        options.otherKey = thisTableName + '_' + this.getPrimaryKey();
      }
    }
    if (options.many == null) {
      options.many = false;
    }
    if (options.as == null) {
      if (otherTableName == null) {
      ...
      options.as = options.many ? otherTableName + 's' : otherTableName;
    }
    return options;
  },
  embed: function(records, otherTable, options) {
    return this.baseEmbed(records, otherTable, this.defaultsForEmbed(otherTable, options));
  },
  queueEmbed: function(otherTable, options) {
    return this.queueAfter(_.partialRight(this.embed, otherTable, options));
  },
  queueEmbedBelongsTo: function(otherTable, immutableOptions) {
    var options;
    options = immutableOptions ? _.clone(immutableOptions) : {};
    if (options.otherIsForeign == null) {
      options.otherIsForeign = false;
    }
    return this.queueEmbed(otherTable, options);
  },
  queueEmbedHasOne: function(otherTable, options) {
    return this.queueEmbed(otherTable, options);
  },
  queueEmbedHasMany: function(otherTable, options) {
    options = immutableOptions ? _.clone(immutableOptions) : {};
    if (options.many == null) {
      options.many = true;
    }
